...
import { fetchTweet, postNoti, postReply } from "../apiCalls";
					...
					<CardContent sx={{ display: "flex", px: 2, pt: 2, pb: 0 }}>
						...
						<Box sx={{ flex: 1 }}>
							...
							<Box sx={{ display: "flex" }}>
								...
								<Box>
									...
									<Typography variant="subtitle1" sx={{ color: "text.fade" }}>
										@{tweet.user[0].handle}
									</Typography>
								</Box>
						...
						<Box>
							<IconButton
								edge="end"
								onClick={
									toggleBottomMenu(true, { _id: tweet._id, owner: tweet.owner })
								}
							>
						...
						>
							<CardActionArea onClick={() => {
								...
								<CardContent sx={{ display: "flex", p: 2 }}>
									<Box sx={{ mr: 3 }}>
										<div onClick={e => {
											navigate("/@" + tweet.origin_tweet[0].user[0].handle);
											e.stopPropagation();
										}}>
											<Avatar
												alt="Profile Picture"
						...
						>
							...
							<CardActionArea onClick={() => {
								...
								<CardContent sx={{ display: "flex", p: 2 }}>
									<Box sx={{ mr: 3 }}>
										<div onClick={e => {
											navigate("/@" + comment.user[0].handle);
											e.stopPropagation();
										}}>
											<Avatar
												alt="Profile Picture"
							...
							>
								...
								<ButtonGroup variant="text">
									<IconButton
										size="small"
										disableRipple={true}
										sx={{ fontSize: "20px", color: "text.fade" }}
										onClick={() => {
											if (auth)
									...
									<Button
										onClick={() => {
											if (comment.likes && comment.likes.length)
												navigate(`/tweet/${comment._id}/likes`);
										}}
										size="small"
										sx={{
											color: "text.fade",
								...
								<ButtonGroup variant="text">
									<IconButton
										...
										onClick={() => {
											if (auth)
												navigate(`/tweet/${comment._id}/share`);
										}}
									>
										<ShareIcon
											sx={{ fontSize: "20px", color: "text.fade" }}
									...
									<Button
										onClick={() => {
											if (comment.shares && comment.shares.length) {
												navigate(`/tweet/${comment._id}/shares`);
											}
										}}
										size="small"
										sx={{
								...
								<ButtonGroup variant="text">
									<IconButton
										...
										sx={{ fontSize: "20px", color: "text.fade" }}
										onClick={() => {
											navigate(`/tweet/${comment._id}#comments`);
										}}
									>
										<ChatBubbleIcon
											sx={{ fontSize: "20px", color: "text.fade" }}
							...
							endAdornment={
								<InputAdornment position="end">
									<IconButton onClick={() => {
										...
										(async () => {
											let result = await postReply(tweet._id, reply);
											if (!result) return navigate("/error");
